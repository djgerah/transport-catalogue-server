...
namespace detail
{
template <typename T> inline void RenderValue(std::ostream &out, const T &value)
{
    out << value;
}
...
template <> inline void RenderValue<std::string>(std::ostream &out, const std::string &s)
{
    HtmlEncodeString(out, s);
}

template <typename AttrType> inline void RenderAttr(std::ostream &out, std::string_view name, const AttrType &value)
{
    out << name << "=\""sv;
...
class Drawable
{
  public:
    ...
    // Объекты Drawable могут удаляться полиморфно. Поэтому деструктор объявляем
    // публичным виртуальным
