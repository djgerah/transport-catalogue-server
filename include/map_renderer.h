...
class MapRenderer
{
  public:
    ...
    std::vector<svg::Polyline> RenderRouteLines(const std::map<std::string_view, const tc::Bus *> &buses,
                                                const SphereProjector &sp) const;
    std::vector<svg::Text> RenderBusLabel(const std::map<std::string_view, const tc::Bus *> &buses,
                                          const SphereProjector &sp) const;
    std::vector<svg::Circle> RenderStopPoints(const std::map<std::string_view, const tc::Stop *> &stops,
                                              const SphereProjector &sp) const;
    std::vector<svg::Text> RenderStopLabel(const std::map<std::string_view, const tc::Stop *> &stops,
                                           const SphereProjector &sp) const;
