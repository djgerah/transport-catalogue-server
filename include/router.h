...
template <typename Weight> graph::VertexId Router<Weight>::GetVertexId(const tc::Stop *stop)
{
    return stop_to_vertex_id_.at(stop);
}

template <typename Weight> const graph::DirectedWeightedGraph<double> &Router<Weight>::GetGraph() const
{
    return graph_;
}
